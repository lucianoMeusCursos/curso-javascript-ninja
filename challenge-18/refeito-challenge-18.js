...
const cleanCPFs = listCpfs.map(cpf => cleanCPF(cpf));
...
*/
const formatCpf = (cpf) => cpf.replace(
    /(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');

console.log( '\nFormatando CPFs corretamente:' );
cleanCPFs.forEach(cpf => console.log(formatCpf(cpf)) )
...
console.log(
  "Os meses de janeiro, junho e julho começam com a letra j.".match(/ju[ln]ho/g));
...
*/
const replaceTags = text => text.replace(/<(\w+)>([^<]+)<\/\w+>/g, '<$1>O texto dentro da tag "$1" é "$2</<$1>\n')
console.log( '\nFazer replace dos textos das tags:' );

console.log(
  replaceTags('<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>')
)
