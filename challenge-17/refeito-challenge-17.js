...
console.log(text.replace(/Manuel Marques de Sousa/, 'Luciano Baraúna'));
...
console.log(
  text.replace(/(O Centauro de Luvas)/g, ($1) => $1.toUpperCase())
);
...
const getMonthNumber = (nameMonth) => {
  const moths = {
    "janeiro": '01',
    "fevereiro": '02',
    "março": '03',
...
listMoths.forEach(mothName => {
  console.log(`O mês de ${mothName} é representado pelo número ${getMonthNumber(mothName)}.`)
});
...
const regexDate = /(\d+) de (junho|julho) de (\d+)/gm;
...
const replaceDate = (regex, day, moth, year) => `${day}/${getMonthNumber(moth)}/${year}`
