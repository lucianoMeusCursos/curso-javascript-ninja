...
const errorMessages = {
  'nomeEmpty': 'Preencha o nome do usuário!',
  'emailEmpty': "Preencha o e-mail!",
  'emailInvalid': "Entre com um e-mail válido!",
  'messageEmpty': "Preencha a mensagem!",
}
...
$button.addEventListener('click', function(event) {
  ...
  if(!$inputUsername.value) {
    showError(errorMessages['nomeEmpty'])
    return;
  }

  if(!$inputEmail.value) {
    showError(errorMessages['emailEmpty'])
    return;
  }

  if(!isValidEmail($inputEmail.value)) {
    showError(errorMessages['emailInvalid'])
    return;
  }

  if(!$message.value) {
    showError(errorMessages['messageEmpty'])
    return;
  }
